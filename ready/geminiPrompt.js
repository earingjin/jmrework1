...
const GEMINI_PROMPT_ITEMS = {
  jobAnalysis: [
    ...
    { content: '희망직무 맞춤형 SWOT 분석', note: '2x2 표 형식으로 출력하되, 입력된 점수와 항목만 근거로 각 칸을 자세히 작성' },
    ...
    { content: '검사 결과에 기반하되, AI시대의 전망과 입력된 학력·자격증까지 고려한 직업 추천 5개', note: '사람인에 나온 직업 위주로, 해당 직업의 채용공고를 유효한 링크로 첨부' },
    { content: '상담사용 핵심 질문 10가지', note: '향후 1:1 심층 상담 시, 내담자의 내면 동기를 이끌어내고 커리어 돌파구를 찾기 위한 코칭 질문 리스트' }
  ],
  interest: [
...
- SWOT은 2x2 표로 만드세요.
- 추천 직업 5개는 직업명, 추천 근거, 준비 과제, 사람인 채용공고 검색 링크를 포함하세요. 입력된 학력과 자격증이 있으면 직업 추천 근거와 준비 과제에 함께 반영하세요.
...
- 과장하지 말고, 검사 결과에서 확인되지 않은 내용은 '추가 확인 필요'라고 표시하세요.`;
  ...
  noTargetInterestUser(input) {
    return `아래 검사 데이터를 기반으로 “직업선호도검사(L형) 진로 탐색형 분석 리포트”를 생성하라. 직업흥미검사만 있어도 리포트를 완성하고, 성격검사와 생활사검사가 있으면 교차검증 근거에 반영한다. 추천직업은 정확히 5개, AI 인생질문은 최소 10개 제시한다. 추천직업은 입력된 학력과 자격증이 있으면 함께 고려하고, 미입력된 학력·자격증은 추측하지 않는다. SWOT은 개인특성 기준으로 작성한다. 지정된 JSON Schema의 핵심 분석 항목만 작성하고 별도 전망 섹션이나 추가 적합도 섹션을 만들지 않는다. HTML 태그, markdown, 코드블록, 설명문 없이 JSON만 반환하라.
...
- swot은 희망직무 기준으로 strengths, weaknesses, opportunities, threats 각각 3~5개를 작성하세요. 각 항목은 입력된 직업흥미 원점수, 입력된 성격검사 점수, 입력된 생활사검사 점수, 전공, 학력, 자격증, 상담사 메모 중 실제 입력된 근거만 연결해 3문장 내외로 자세히 설명하세요.
- swot에서 입력되지 않은 성격검사, 생활사검사, 학력, 자격증, 전공, 상담사 메모는 추측하지 말고 근거로 사용하지 마세요.
- recommendedJobs는 희망직무와 검사 결과를 기준으로 하되, 입력된 학력과 자격증을 함께 고려해 5개만 추천하세요. 학력 또는 자격증이 미입력된 경우 해당 조건은 "추가 확인 필요"로 처리하고 임의로 가정하지 마세요.
- 자격증이 입력된 경우에는 targetJobCompetencyAnalysis, swot, recommendedJobs, finalStrategy에 자격증 활용 방안을 반영하세요.
...
- swot의 각 항목은 단순 키워드가 아니라 희망직무와의 관련성, 입력 점수 근거, 보완 또는 활용 방향을 포함해 3문장 내외로 작성하세요.
- recommendedJobs는 반드시 5개만 작성하고, 각 항목은 title, reason, preparation만 채우세요. reason에는 검사 점수 근거와 입력된 학력·자격증 고려 내용을 포함하고, preparation에는 학력·자격증 기준에서 필요한 준비 과제를 포함하세요.
...
- finalStrategy는 흥미검사, 입력된 성격검사, 입력된 생활사검사, 인적사항, 자격증, 상담사 메모를 종합하여 자세히 작성하되 미입력 자료는 추정하지 마세요.
